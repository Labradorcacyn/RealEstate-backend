...
import salesianos.triana.dam.RealEstate.dto.inmobiliariaDto.InmobiliariaEscuetoDto;
import salesianos.triana.dam.RealEstate.dto.inmobiliariaDto.InmobiliariaEscuetoDtoConverter;
import salesianos.triana.dam.RealEstate.dto.sharedDto.InmobiliariaViviendasDto;
import salesianos.triana.dam.RealEstate.dto.viviendaDto.InmobiliariaViviendasDtoConverter;
...
public class InmobiliariaController {
    ...
    @Operation(summary = "Encontrar los gestores de una inmobiliaria")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200",
                    description = "Se ha encontrado",
                    content = { @Content(mediaType = "application/json")}),
    ...
    public ResponseEntity<List<Usuario>> findByGestoresVivi(@PathVariable UUID id, @AuthenticationPrincipal Usuario u){
        Optional<Inmobiliaria> i = inmobiliariaService.findById(id);


        if (i.isEmpty())
            return ResponseEntity.notFound().build();
        Inmobiliaria ni = i.get();

        if (!ni.getGestores().contains(u) && u.getRole()!= UserRole.ADMIN){
            return ResponseEntity.status(HttpStatus.FORBIDDEN).build();
        }
        List<Usuario> gestores = ni.getGestores();
